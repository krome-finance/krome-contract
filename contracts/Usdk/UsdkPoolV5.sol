...
interface IAMOMinterRegistry {
    function isAMOMinter(address _address) external view returns (bool);
}

contract UsdkPoolV5 is LocatorBasedProxyV2 {
    ...
    IAMOMinterRegistry public amo_minter_registry;
    ...
    uint256 public redemption_delay; // Number of blocks to wait before being able to collectRedemption()
    ...
    modifier onlyByOwnGovCust() {
        require(msgByManager() || msg.sender == custodian_address, "Not owner, tlck, or custd");
        _;
    }

    modifier onlyAMOMinters() {
        require(amo_minter_registry.isAMOMinter(msg.sender), "Not an AMO Minter");
        _;
    }
    ...
    function initialize (
        ...
        // set default values
        redemption_delay = 2; // Number of blocks to wait before being able to collectRedemption()
        redeem_price_threshold = 990000; // $0.99
        mint_price_threshold = 1010000; // $1.01

        bbkMaxColE18OutPerHour = 1000e18;
    ...
    function syncLocator() public {
        LocatorBasedProxyV2.syncOwnership();
    ...
    function curEpochHr() internal view returns (uint256) {
    ...
    function amoMinterBorrow(uint256 col_idx, uint256 collateral_amount) external onlyAMOMinters {
        require(col_idx < collateral_addresses.length && collateral_addresses[col_idx] != address(0), "invalid col_idx");
        // Transfer
        TransferHelper.safeTransfer(collateral_addresses[col_idx], msg.sender, collateral_amount);
    }
    ...
    // Add an AMO Minter
    function setAMOMinterRegistry(address _registry_address) external onlyByOwnGov {
        amo_minter_registry = IAMOMinterRegistry(_registry_address);
        require(!amo_minter_registry.isAMOMinter(address(0)), "Invalid amo minter registry");

        emit SetAMOMinterRegistry(_registry_address);
    }
    ...
    function _addCollateral(
        ...
        require(collateralAddrToIdx[_collateral_address] == 0 && (collateral_addresses.length == 0 || collateral_addresses[0] != _collateral_address), "duplicated");
    ...
    event SetAMOMinterRegistry(address amo_minter_registry);
