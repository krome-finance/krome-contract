...
contract StakingRewardComptrollerV4 is LocatorBasedProxyV2, ReentrancyGuardUpgradeable {
    ...
    // Instances
    IStakingTreasuryV2 private treasury;
    ...
    function initialize (
        ...
        nextRewardRatesManual = _rewardRatesManual;

        rewardsDuration = 604800; // 7 * 86400  (7 days)

        source_comptroller = ISourceComptroller(_source_comptroller);
        ve_multiplier_source = IVeMultiplierSource(_ve_multiplier_source);

        uint256 _source_reward_token_length = 0;
        if (address(source_comptroller) != address(0)) {
            uint256[] memory _source_reward_rewards = source_comptroller.rewardsPerWeight();
            _source_reward_token_length = _source_reward_rewards.length;
        }
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++){ 
            ...
            if (address(source_comptroller) != address(0) && i < _source_reward_token_length) {
    ...
    function rewardsPerWeight() public view returns (uint256[] memory newRewardsPerWeightStored) {
        if (!source_synced && address(source_comptroller) != address(0)) {
            ...
            uint256[] memory sourceRewardsPerWeight = source_comptroller.rewardsPerWeight();
            for (uint i = 0; i < sourceRewardsPerWeight.length; i++) {
                newRewardsPerWeightStored[i] = sourceRewardsPerWeight[i];
            }
    ...
    function collectRewardFor(address rewardee, address destination_address) external nonReentrant returns (uint256[] memory rewards_before) {
        ...
        if (treasury.lockedLiquidityOf(rewardee) > 0 || (address(source_comptroller) != address(0) && !user_source_synced[rewardee])) {
    ...
    function addRewardToken(address _token_address, address _reward_manager, uint256 _reward_rate_manual, address _gauge_controller) external onlyByOwnGov {
        uint256 idx = rewardTokens.length;

        rewardTokens.push(_token_address);
        gaugeControllers.push(_gauge_controller);
        rewardRatesManual.push(_reward_rate_manual);

        rewardTokenAddrToIdx[_token_address] = idx;
        ...
        // Initialize the reward managers
        rewardManagers[_token_address] = _reward_manager;
