...
abstract contract LpMigrationTreasury_ERC20 is ContextUpgradeable, ReentrancyGuardUpgradeable, LocatorBasedProxyV2 {
    ...
    uint256 public should_lock_until; // minimum lock end time that stakes should be locked
    ...
    function __StakingTreasury_init(
        ...
        __StakingTreasury_init_unchained(_staking_boost_controller, _staking_token, _closed_at, _minimum_lockend);
    }
    ...
    function __StakingTreasury_init_unchained(
        ...
        should_lock_until = _minimum_lockend;
    ...
    function extendLockTime(bytes32 kek_id, uint256 secs) updateRewardAndBalance(msg.sender, true) public {
        ...
        require(block.timestamp + secs >= should_lock_until, "too short lock time");
    ...
    function migrator_set_migrated(address staker_address) external {
        require((staker_allowed_migrators[staker_address][msg.sender] && valid_migrators[msg.sender]) || msgByManager(), "Mig. invalid or unapproved");
        migrated[staker_address] = true;
    }
