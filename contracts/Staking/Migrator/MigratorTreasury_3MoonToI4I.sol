...
contract MigratorTreasury_3MoonToI4I is LocatorBasedProxyV2 {
    ...
    uint256 public max_migrate_count;
    ...
    function setMaxMigrateCount(uint256 _v) external onlyByManager() {
        max_migrate_count = _v;
    }

    /* =========== MAIN METHODS ============= */

    function migrate(address _pool) external {
        require(_pool == address(source_treasury), "Invalid pool");
        _migrate(msg.sender);
    }
    ...
    function migrateAccount(address _account) external onlyByManager {
        _migrate(_account);
    }
